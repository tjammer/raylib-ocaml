include Raygui_generated_constants
...
let prop_to_int = function
  |   `Border_color_normal -> ControlProperty.(to_int Border_color_normal)
    ...
    | `Dropdown_items_padding -> DropdownBoxProperty.(to_int Dropdown_items_padding)
    ...
    | `Huebar_selector_height -> ColorPickerProperty.(to_int Huebar_selector_height)
    | `Huebar_selector_overflow -> ColorPickerProperty.(to_int Huebar_selector_overflow)



let get_style = function
  | Control.Default prop -> _get_style Wrapped_Control.Default  (prop_to_int prop)
...
let set_style ctl value =
  match ctl with
  | Control.Default prop -> _set_style Wrapped_Control.Default  (prop_to_int prop) value
