...
let int_of_light_type = function Directional -> 0 | Point -> 1
...
let update_light_values shader light =
  ...
  set_shader_value shader light.loc_enabled (to_voidp enabled)
  ...
  set_shader_value shader light.loc_pos
    (to_voidp (addr light.position))
    ShaderUniformDataType.Vec3;
  set_shader_value shader light.loc_target
  ...
  in
  set_shader_value shader light.loc_color
    (to_voidp (addr color))
    ShaderUniformDataType.Vec4

let create_light typ position target color shader =
  if !lights_count > max_lights then failwith "Too many lights"
  else
